...
Tags: widget, customization, theme
...
*This plugin is **NOT** required for WordPress 2.2*

== Installation ==

If you are using WordPress version 2.2 or later, you do not need to install this
plugin;  WordPress itself already includes these features.
...
complicated. The default `before_widget` includes sprintf directives `%1$s` and `%2$s`,
...
= The Seach Widget is ugly.  I want my theme's original search box as a widget. =

That's not a question, but it is a frequently formed comment, so we'll respond anyway.
...
= Great, I have the HTML markup looking good, but it looks awful in the browser! =

From questions, to statements, to exclamations.  You'r getting pretty antsy, aren't you?

But, yeah, we knew that would happen. Your theme was probably written before widgets
...
Oh, that's easy. Instead of `register_sidebar()` you should use `register_sidebars(n)`
where `n` is the number of sidebars. Then place the appropriate number in
the `dynamic_sidebar()` function, starting with 1. There are several ways to dse
